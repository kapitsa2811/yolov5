...
class Detect(nn.Module):
    ...
    def forward(self, x):
        ...
        for i in range(self.nl):
            ...
            bs, _, ny, nx = x[i].shape  # x(bs,255,20,20) to x(bs,3,20,20,85)# &&& now what is ny,ny and intention for this opeation??
            x[i] = x[i].view(bs, self.na, self.no, ny, nx).permute(0, 1, 3, 4, 2).contiguous() # &&&& waht is size after this operation

            if not self.training:  # inference. ##&&& need to understand this
    ...
    def _make_grid(nx=20, ny=20):
        yv, xv = torch.meshgrid([torch.arange(ny), torch.arange(nx)]) # &&& https://www.kite.com/python/docs/torch.meshgrid 
...
class Model(nn.Module):
    def __init__(self, cfg='yolov5s.yaml', ch=3, nc=None, anchors=None):  # model, input channels, number of classes
        ...
        if nc and nc != self.yaml['nc']:  ## &&& loading number of classes and anchors
